...
def client_cli_name1(cli_no):
    sql_exp = f'SELECT cli_name1 FROM client WHERE cli_no = {cli_no}'
...
def order_not_reserved(ref, client):
    # if user != 'SANJAY':
    #     to_list = ['sales@quatroair.com']
...
def alert_handler(alert, ref, user):
    if alert == 'BO ALLOWED':
        ...
        client = order_cli_name1(ref)
        body, to_list, cc_list, subject_str = order_not_reserved(ref, client)
        email_handler(body, to_list, cc_list, subject_str)
